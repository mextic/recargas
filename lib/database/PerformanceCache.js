...
class PerformanceCache {
    ...
    async initialize() {
        ...
        if (this.isRedisAvailable) {
            try {
                // Test Redis connection - works with both pooled and traditional Redis clients
                if (this.redis.ping) {
                    await this.redis.ping();
                } else if (this.redis.getConnection) {
                    // Handle pooled Redis client
                    const conn = await this.redis.getConnection();
                    await conn.ping();
                    conn.release();
                }
                console.log('   ✅ Redis disponible para cache (optimized connection pooling)');
            } catch (error) {
        ...
        console.log(`   📊 Cache configurado: ${this.isRedisAvailable ? 'Redis (Pooled)' : 'Memoria'}`);
    }
        ...
        try {
            ...
            if (this.isRedisAvailable) {
                ...
                if (this.redis.setEx) {
                    ...
                    // Fallback for Redis clients without setEx
    ...
    async invalidatePattern(pattern) {
        try {
            if (this.isRedisAvailable) {
                ...
                if (keys.length > 0) {
                    // Fix: spread keys for multi-key deletion
    ...
    /**
     * Scan Redis keys using SCAN instead of KEYS for production safety
    ...
    async _scanKeys(pattern) {
        ...
        if (this.redis.scan) {
            ...
            try {
                do {
                    const result = await conn.client.scan(cursor, 'MATCH', pattern, 'COUNT', 100);
                    cursor = result[0];
                    keys.push(...result[1]);
                } while (cursor !== 0);
            } finally {
    ...
    async getHealthStatus() {
        ...
        return {
            ...
            backend: this.isRedisAvailable ? 'Redis (Pooled)' : 'Memory',
            stats,
