...
`;
        ...
        weeklyData.data.forEach((week, index) => {
            ...
            Object.entries(week.services).forEach(([serviceName, data]) => {
                ...
                // Mostrar estados de saldo para GPS
                if (serviceName.toLowerCase() === 'gps' && data.saldoStates) {
                    output += `     📊 ESTADOS DE SALDO:\n`;
                    output += `       ❌ Vencidos: ${data.saldoStates.vencidos.count} (${data.saldoStates.vencidos.percentage}%)\n`;
                    output += `       🟡 Por vencer hoy: ${data.saldoStates.porVencer.count} (${data.saldoStates.porVencer.percentage}%)\n`;
                    output += `       ✅ Vigentes: ${data.saldoStates.vigentes.count} (${data.saldoStates.vigentes.percentage}%)\n`;
                }
...
`;
        ...
        monthlyData.data.slice(0, 3).forEach((month, index) => {
            ...
            Object.entries(month.services).forEach(([serviceName, data]) => {
                ...
                // Mostrar estados de saldo para GPS (solo en el mes actual)
                if (serviceName.toLowerCase() === 'gps' && data.saldoStates && month.isCurrentMonth) {
                    output += `     📊 ESTADOS DE SALDO (ACTUAL):\n`;
                    output += `       ❌ Vencidos: ${data.saldoStates.vencidos.count} (${data.saldoStates.vencidos.percentage}%)\n`;
                    output += `       🟡 Por vencer hoy: ${data.saldoStates.porVencer.count} (${data.saldoStates.porVencer.percentage}%)\n`;
                    output += `       ✅ Vigentes: ${data.saldoStates.vigentes.count} (${data.saldoStates.vigentes.percentage}%)\n`;
                }
