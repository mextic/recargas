...
class ELIoTRechargeProcessor extends BaseRechargeProcessor {
    ...
    /**
     * Filtra registros ELIoT para separar los que requieren recarga vs los que están en ahorro
     * NOTA: En ELIoT, el filtrado principal ya se hace en getRecordsToProcess() con MongoDB
     * @param {Array} records - Registros ya pre-filtrados por MongoDB
     * @returns {Object} - { toRecharge: [], savings: [], metrics: {} }
     */
    async filterRecordsForRecharge(records) {
        // En ELIoT, los registros que llegan ya han sido filtrados por MongoDB
        // en el método getRecordsToProcess() -> filterByMongoMetrics()
        // Por lo tanto, TODOS los registros que llegan requieren recarga
        ...
        records.forEach(record => {
            const fechaSaldo = parseInt(record.fecha_saldo);

            if (fechaSaldo < ahoraUnix) {
                vencidos++;
            } else if (fechaSaldo >= ahora.clone().startOf('day').unix() && fechaSaldo <= finDiaHoy) {
                porVencer++;
            } else {
                vigentes++;
            }
        ...
        const metrics = {
            ...
            porVencer,
            vigentes,
            algorithm: 'ELIOT_MONGODB_PREFILTERED',
            minutosUmbral: this.config.MINUTOS_SIN_REPORTAR_PARA_RECARGA
        };

        this.logger.info('Filtrado ELIoT completado en BaseRechargeProcessor', {
            operation: 'eliot_filtering_base_processor',
            ...
            savings: 0,
            note: 'All records pre-filtered by MongoDB, no savings at this stage'
        });

        return {
            toRecharge: records, // Todos los registros necesitan recarga
            savings: [], // En ELIoT, el ahorro se detecta después con MongoDB
            metrics
        };
    }
    ...
    async processRecords(records, stats) {
        ...
        console.log(`📊 [ELIoT] Iniciando procesamiento de ${registrosArecargar.length} dispositivos con proveedor: ${provider.name} (Saldo: $${provider.balance})`);
            ...
            try {
                ...
                if (rechargeResult.success) {
                    ...
                    await this.executeWithRetry(
                        async () => await this.persistenceQueue.addToAuxiliaryQueue(auxItem, 'eliot'),
                        {
        ...
        console.log(`✅ [ELIoT] Completado: ${stats.successful} exitosos, ${stats.failed} errores en ${elapsedTime > 60 ? Math.floor(elapsedTime / 60) + 'm ' + (elapsedTime % 60) + 's' : elapsedTime + 's'}`);
    ...
    /**
     * Inserta recargas en lote con manejo de duplicados para ELIoT
     * Similar a GPS y VOZ pero adaptado para estructura de datos ELIoT
     */
    async insertBatchRechargesWithDuplicateHandling(recharges, isRecovery = false) {
        const results = { inserted: [], duplicates: [], errors: [] };
        const serviceType = this.getServiceType();

        this.logger.info('Iniciando inserción batch con manejo de duplicados ELIoT', {
        ...
        try {
            // Calcular totales específicos para ELIoT
            const totalRecargas = recharges.length;
            const totalImporte = recharges.reduce((sum, r) => sum + (r.importe || 0), 0);
            ...
            if (isRecovery) {
                ...
                this.logger.info('Aplicando prefijo de recuperación', {
                    operation: 'recovery_prefix_applied',
                    serviceType,
                    originalNote: `[ ${String(totalRecargas).padStart(3, '0')} / ${String(totalRecargas).padStart(3, '0')} ] Recarga Automática: ELIoT`,
            ...
            this.logger.info('Registro maestro ELIoT insertado', {
            ...
            for (const recharge of recharges) {
                try {
                    ...
                    if (!folio || !sim) {
                        ...
                        this.logger.error('Datos incompletos para inserción ELIoT', {
                            sim: sim,
                            folio: folio,
                    ...
                    // Generar el detalle completo con información de la transacción ELIoT
                    ...
                    const importe = recharge.importe || 0;

                    // Para ELIoT agregar información específica del agente y paquete variable
                    const importeRecarga = recharge.record?.importe_recarga || importe;
                    const diasRecarga = recharge.record?.dias_recarga || 'N/A';
                    const uuid = recharge.record?.uuid || 'N/A';

                    const detalleText = `[ Saldo Final: ${saldoFinal} ] Folio: ${folio}, Cantidad: $${importeRecarga}, Teléfono: ${sim}, Carrier: ${carrier}, Fecha: ${fechaRecarga}, TransID: ${transID}, Timeout: ${timeout}, IP: ${ip}, Días: ${diasRecarga}, UUID: ${uuid}`;

                    // Construir información del vehículo directamente desde record
                    const descripcion = recharge.record?.descripcion || recharge.record?.dispositivo || 'Agente';
                    const empresa = recharge.record?.empresa || 'N/A';
                    const vehiculoFormatted = `${descripcion} [${empresa}]`;
                    ...
                    await this.db.querySequelize(
                         ...
                         VALUES (?, ?, ?, ?, ?, ?, ?, 1)`,
                        {
                            replacements: [
                                ...
                                sim,
                                importeRecarga,
                                recharge.record?.uuid || 'N/A',  // Usar uuid del record ELIoT
                                vehiculoFormatted,
                                detalleText,
                                folio
                            ],
                    ...
                    this.logger.debug('Detalle ELIoT insertado exitosamente', {
                        sim: sim,
                        folio: folio,
                        importe: importeRecarga,
                        diasRecarga: diasRecarga,
                        idRecarga: idRecarga
                    });
                    ...
                    if (error.name === 'SequelizeUniqueConstraintError' ||
                        ...
                        this.logger.info('Folio duplicado detectado - ya procesado anteriormente', {
                            sim: recharge.sim,
                            folio: recharge.webserviceResponse?.folio,
                            message: 'Tratando como exitoso - ya está en BD'
                        });
                    } else {
                        // ERROR REAL (NO duplicado) - DEBE HACER ROLLBACK
                        results.errors.push(recharge);
                        this.logger.error('ERROR CRÍTICO insertando detalle ELIoT', error, {
                            sim: recharge.sim,
                            folio: recharge.webserviceResponse?.folio
                        });

                        // 🔴 MODO PARANOICO: Error NO duplicado = FATAL - ROLLBACK
                        throw new Error(`Atomicidad violada ELIoT: Error insertando detalle para SIM ${recharge.sim}: ${error.message}`);
                    }
                }
            }

            // Actualizar fechas de expiración ELIoT usando datos variables del record
            this.logger.info('Actualizando fechas de expiración ELIoT', {
            ...
            for (const recharge of results.inserted) {
                const moment = require('moment-timezone');
                const diasRecarga = recharge.record?.dias_recarga || 7; // Default 7 días si no se especifica
                // CRÍTICO: Usar endOf('day') para fijar al final del día
                const nuevaFechaExpiracion = moment().tz('America/Mazatlan').add(diasRecarga, 'days').endOf('day').unix();
                ...
                await this.dbEliot.querySequelize(
                    `UPDATE agentes SET fecha_saldo = ? WHERE sim = ?`,
                    {
                        ...
                        type: this.dbEliot.getSequelizeClient().QueryTypes.UPDATE,
                        transaction: null // ELIoT updates in separate DB, no transaction
                    }
                );

                this.logger.debug('Fecha expiración ELIoT actualizada', {
                    sim: recharge.sim,
                    diasRecarga,
            ...
            // 🔒 LIMPIEZA SELECTIVA DE COLA AUXILIAR (MODO PARANOICO)
            // Solo limpiar items exitosos (inserted + duplicates), mantener errors para retry
            const itemsToRemove = [...results.inserted, ...results.duplicates];

            if (itemsToRemove.length > 0) {
                this.logger.info('Iniciando limpieza selectiva de cola auxiliar ELIoT', {
                    operation: 'selective_queue_cleanup_start',
                    serviceType,
                ...
                // VALIDACIÓN PARANOICA: Verificar que realmente están en BD antes de limpiar
                const validatedItems = [];
                for (const item of itemsToRemove) {
                    const folio = item.webserviceResponse?.folio;
                    if (!folio) {
                        this.logger.warn('Item sin folio - omitiendo validación', {
                            sim: item.sim
                        });
                    ...
                    try {
                        const exists = await this.checkFolioExists(folio, item.sim);
                        if (exists) {
                            validatedItems.push(item);
                        } else {
                            this.logger.error('PARANOIA ACTIVADA: Folio marcado exitoso pero NO está en BD', {
                                folio: folio,
                                sim: item.sim,
                                status: results.duplicates.includes(item) ? 'duplicate' : 'inserted'
                            });
                        }
                    } catch (validationError) {
                        this.logger.error('Error validando folio en BD', validationError, {
                            folio: folio,
                            sim: item.sim
                        });
                    }
                ...
                // Obtener SIMs de items validados para limpiar
                const successfulSims = new Set(validatedItems.map(r => r.sim));

                // Filtrar cola auxiliar - mantener SOLO los que fallaron
                const originalQueueLength = this.persistenceQueue.auxiliaryQueue.length;
                this.persistenceQueue.auxiliaryQueue = this.persistenceQueue.auxiliaryQueue.filter(item => {
                    // Mantener si:
                    // 1. No es del servicio actual (ELIoT)
                    if (item.tipo !== 'ELIoT_recharge') {
                        return true; // Mantener items de otros servicios
                    }

                    // 2. Es del servicio actual pero NO está en los exitosos
                    return !successfulSims.has(item.sim);
                });

                // Guardar cola actualizada
                await this.persistenceQueue.saveQueueToDisk();

                this.logger.info('Limpieza selectiva completada', {
                    operation: 'selective_queue_cleanup_complete',
                    serviceType,
                    originalQueueLength,
                    finalQueueLength: this.persistenceQueue.auxiliaryQueue.length,
                    removedItems: validatedItems.length,
                    keptItems: results.errors.length
                });
            }

            // Log de items que permanecen en cola para retry
            if (results.errors.length > 0) {
                this.logger.warn('Items con errores permanecen en cola auxiliar para retry', {
                    operation: 'items_kept_for_retry',
                    serviceType,
                    errorCount: results.errors.length,
                    errorSims: results.errors.map(e => e.sim)
                });
            }

            this.logger.info('Lote ELIoT insertado exitosamente con manejo de duplicados', {
                operation: 'eliot_batch_insert_with_duplicates_success',
                serviceType,
                totalRecargas,
                idRecarga,
                totalImporte,
            ...
            if (transaction) await transaction.rollback();
            this.logger.error('Error insertando lote ELIoT con manejo de duplicados', error, {
                operation: 'eliot_batch_insert_with_duplicates_error',
                serviceType,
