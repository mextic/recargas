...
class BaseRechargeProcessor {
    ...
    async processAuxiliaryQueueRecharges() {
        ...
        try {
            ...
            // VALIDACIÓN EXHAUSTIVA: Solo limpiar recargas 100% VERIFICADAS en BD
            if (stats.processed > 0) {
                console.log(`   🔍 VALIDACIÓN CRÍTICA: Verificando ${stats.processed} recargas antes de limpiar cola...`);

                // DOBLE VALIDACIÓN: Re-verificar que las recargas están realmente en BD
                const rechargesForValidation = pendingRecharges.filter(recharge =>
                    processedSims.has(recharge.sim)
                );

                const { verified: doubleVerified, notVerified: stillNotVerified } =
                    await this.validateRechargesInDB(rechargesForValidation);

                if (stillNotVerified.length > 0) {
                    this.logger.error('ALERTA CRÍTICA: Recargas marcadas como procesadas NO están en BD', {
                        operation: 'critical_validation_failed',
                        serviceType,
                        claimed: stats.processed,
                        actuallyVerified: doubleVerified.length,
                        stillMissing: stillNotVerified.length,
                        missingSims: stillNotVerified
                    });

                    console.error(`   ❌ CRÍTICO: ${stillNotVerified.length}/${stats.processed} recargas marcadas como exitosas NO están en BD`);
                    console.error(`   🚫 NO se limpiará la cola hasta verificar 100% de inserción`);

                    // NO limpiar la cola si hay discrepancias
                    return stats;
                }

                console.log(`   ✅ VALIDACIÓN OK: ${doubleVerified.length} recargas verificadas al 100% en BD`);

                const originalLength = this.persistenceQueue.auxiliaryQueue.length;

                // Solo eliminar recargas DOBLE-VERIFICADAS
                this.persistenceQueue.auxiliaryQueue = this.persistenceQueue.auxiliaryQueue.filter(item => {
                    if (item.tipo === `${serviceType}_recharge` && doubleVerified.includes(item.sim)) {
                        console.log(`   🗑️ Limpiando ${item.sim} (folio: ${item.transId}) - Verificado en BD`);
                        return false; // Remover SOLO verificados
                    }
                ...
                await this.executeWithRetry(
                    ...
                    {
                        operationName: 'save_auxiliary_queue_verified',
                        transactionId: `cleanup_verified_${Date.now()}`
                    }
                );
