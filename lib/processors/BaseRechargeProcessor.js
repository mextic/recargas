        ...
        try {
            ...
            if (pendingStats.auxiliaryQueue.pendingDb > 0) {
                ...
                if (recoveryResult.failed > 0) {
                    ...
                    // NO procesar nuevos registros, pero DEBE liberar lock en finally
                }
            } else {
                ...
                if (records.length > 0) {
                    ...
                    const processResult = await this.processRecords(records, stats);
    ...
    async processAuxiliaryQueueRecharges() {
        ...
        try {
            ...
            if (this.insertBatchRecharges && typeof this.insertBatchRecharges === 'function') {
                ...
                try {
                    console.log(`   🔄 Procesando ${pendingRecharges.length} recargas ${serviceType.toUpperCase()} como LOTE de recovery...`);
                    await this.insertBatchRecharges(pendingRecharges);
                    
                    stats.processed = pendingRecharges.length;
                    pendingRecharges.forEach(recharge => processedSims.add(recharge.sim));
                    console.log(`   ✅ LOTE ${serviceType.toUpperCase()} recovery: ${pendingRecharges.length} recargas insertadas como un solo registro maestro`);
                } catch (error) {
                    stats.failed = pendingRecharges.length;
                    console.error(`   ❌ Error procesando lote ${serviceType.toUpperCase()} recovery:`, error.message);
                }
            } else {
