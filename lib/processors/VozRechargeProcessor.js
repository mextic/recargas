...
class VozRechargeProcessor extends BaseRechargeProcessor {
        ...
        for (let i = 0; i < records.length; i++) {
            ...
            try {
                ...
                if (!paqueteConfig) {
                    ...
                    progressBar.update(i + 1, `❌ Error: SIM ${record.sim} - Código desconocido`);
                    stats.failed++;
                    continue;
                }

                // Actualizar progreso con información detallada
                progressBar.update(i, `📞 SIM ${record.sim} - ${paqueteConfig.descripcion} ($${paqueteConfig.monto})`);
                ...
                if (rechargeResult.success) {
                    ...
                    progressBar.update(i + 1, `✅ SIM ${record.sim} - Recargado exitosamente`);
                    ...
                    progressBar.update(i + 1, `❌ SIM ${record.sim} - Error: ${rechargeResult.error}`);
    ...
    async insertBatchRecharges(recharges) {
        ...
        try {
            ...
            // Nota del registro maestro VOZ con formato correcto
            const masterNote = `[ ${String(totalRecargas).padStart(3, '0')} / ${String(totalRecargas).padStart(3, '0')} ] Recarga Automática VOZ - ${totalRecargas} paquetes procesados`;
