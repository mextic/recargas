/**
 * HealthCheckManager - FASE 7: Sistema de Health Checks con Resiliencia
 * Monitoreo proactivo de servicios críticos con alertas automáticas
 * Integrado con EventBus y Circuit Breakers
 */
...
const { getCircuitBreakerManager } = require('../resilience/CircuitBreaker');
const { getRetryStrategyManager } = require('../resilience/RetryStrategy');
...
class HealthCheckManager {
    constructor() {
        ...
        // Integración con sistemas de resiliencia
        this.eventBus = getEventBus();
        this.circuitBreakerManager = getCircuitBreakerManager();
        this.retryManager = getRetryStrategyManager();
        ...
        this.config = {
            ...
            resilience: {
                enableCircuitBreaker: process.env.HEALTH_CIRCUIT_BREAKER_ENABLED !== 'false',
                enableRetry: process.env.HEALTH_RETRY_ENABLED !== 'false',
                maxRetries: parseInt(process.env.HEALTH_MAX_RETRIES) || 2,
                circuitFailureThreshold: parseInt(process.env.HEALTH_CB_FAILURE_THRESHOLD) || 3
            }
        };
        ...
        console.log('🏥 HealthCheckManager inicializado con resiliencia');
        console.log(`⏰ Intervalos configurados:`, this.config.intervals);
        console.log(`🔧 Resiliencia:`, {
            circuitBreaker: this.config.resilience.enableCircuitBreaker,
            retry: this.config.resilience.enableRetry
        });
    }
    ...
    initializeResilienceComponents() {
        if (this.config.resilience.enableCircuitBreaker) {
            // Circuit breakers para servicios externos
            this.circuitBreakerManager.getCircuitBreaker('health_taecel', {
                failureThreshold: this.config.resilience.circuitFailureThreshold,
                recoveryTimeout: 30000, // 30 segundos
                monitoringPeriod: 60000 // 1 minuto
            });

            this.circuitBreakerManager.getCircuitBreaker('health_mst', {
                failureThreshold: this.config.resilience.circuitFailureThreshold,
                recoveryTimeout: 30000,
                monitoringPeriod: 60000
            });

            this.circuitBreakerManager.getCircuitBreaker('health_database', {
                failureThreshold: 2, // Más sensible para BD
                recoveryTimeout: 15000, // Recuperación más rápida
                monitoringPeriod: 60000
            });

            console.log('🔒 Circuit breakers para health checks configurados');
        }

        if (this.config.resilience.enableRetry) {
            // Estrategia específica para health checks
            this.retryManager.getStrategy('health_check', {
                maxAttempts: this.config.resilience.maxRetries,
                baseDelay: 1000,
                maxDelay: 5000,
                multiplier: 1.5,
                jitterType: 'EQUAL'
            ...
            console.log('🔄 Retry strategy para health checks configurada');
        }
    }
