 ...
 */
const { dbGps, dbEliot, getRedisClient, getPoolStats, checkDatabaseHealth } = require('../../database');
...
class DatabaseHealthCheck {
        ...
        try {
            ...
            return {
                ...
                details: {
                    ...
                    connectionPool: dbGps.getPoolStats ? dbGps.getPoolStats() : this.getConnectionPoolInfo(dbGps)
                }
            };
        ...
        try {
            ...
            return {
                ...
                details: {
                    ...
                    connectionPool: dbEliot.getPoolStats ? dbEliot.getPoolStats() : this.getConnectionPoolInfo(dbEliot)
                }
            };
    ...
    async checkRedis() {
        ...
        try {
            ...
            // Test de escritura/lectura with connection pool support
            const testKey = 'health_check_test';
            const testValue = Date.now().toString();

            // Handle both pooled and traditional Redis clients
            ...
            let poolStats = null;
            try {
                if (redisClient.getPoolStats) {
                    poolStats = redisClient.getPoolStats();
                }
            } catch (e) {
            ...
            return {
                ...
                details: {
                    ...
                    connected: redisClient.isReady || (redisClient.ping !== undefined),
                    connectionPool: poolStats ? poolStats.redis : null
                }
            };
    ...
    getConnectionPoolInfo(dbConnection) {
        try {
            ...
            const details = {
                ...
                acquired: pool.borrowed ?? pool.acquired ?? (pool.size - pool.available),
    ...
    async testAllConnections() {
        ...
        Object.entries(results).forEach(([db, result]) => {
            ...
            if (result.details && result.details.connectionPool) {
                console.log(`      Pool: ${JSON.stringify(result.details.connectionPool, null, 2)}`);
            }
        });

        // Also test the enhanced database health check function
        try {
            const enhancedResults = await checkDatabaseHealth();
            console.log('🔍 Enhanced Pool Health Check:');
            console.log(JSON.stringify(enhancedResults, null, 2));
        } catch (error) {
            console.log('⚠️ Enhanced health check not available:', error.message);
        }
