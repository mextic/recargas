...
const { getDeadLetterQueueManager } = require('../resilience/DeadLetterQueue');
const { getEventBus } = require('../events/EventBus');
const { EventTypes } = require('../events/EventTypes');

/**
 * Cliente centralizado para webservices TAECEL y MST con resiliencia
 * Incluye Circuit Breakers, Retry Strategy y Dead Letter Queue
 */
class WebserviceClient {
    constructor() {
        ...
        this.dlqManager = getDeadLetterQueueManager();
        this.eventBus = getEventBus();

        // Configurar circuit breakers para webservices
    ...
    initializeCircuitBreakers() {
        // Circuit breaker para TAECEL
        this.taecelCB = this.circuitBreakerManager.getCircuitBreaker('taecel_api', {
            failureThreshold: 3,
            recoveryTimeout: 60000, // 1 minuto
            monitoringPeriod: 300000, // 5 minutos
            slowCallThreshold: 10000, // 10 segundos
            slowCallRateThreshold: 0.6
        });

        // Circuit breaker para MST
        this.mstCB = this.circuitBreakerManager.getCircuitBreaker('mst_soap', {
            failureThreshold: 2,
            recoveryTimeout: 30000, // 30 segundos
            monitoringPeriod: 300000,
            slowCallThreshold: 15000, // 15 segundos
            slowCallRateThreshold: 0.5
        });

        // Obtener estrategias de retry
        this.webserviceRetry = this.retryManager.getStrategy('webservice');

        console.log('🔒 Circuit breakers para webservices configurados');
    }
    // ===== TAECEL METHODS =====
    ...
    static async taecelRecharge(sim, codigoPaquete) {
        try {
            // Solo mostrar logs si NO estamos en modo progreso activo
