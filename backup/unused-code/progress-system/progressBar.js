...
class ProgressBar {
    constructor(options = {}) {
        ...
        // Activar modo silencioso global para evitar interferencia de console.log
    ...
    complete(message = 'Completado') {
        ...
        // Desactivar modo silencioso cuando la barra termina
        global.PROGRESS_ACTIVE = false;
        // Agregar nueva línea para separar del siguiente output
        process.stdout.write('\n');

        // Flush de logs buffereados
        try {
            const { flushLogBuffer } = require('./logger');
            flushLogBuffer();
        } catch (error) {
            // Silenciar error si no se puede cargar logger
        }
    }
    ...
    fail(message = 'Error') {
        this.message = `❌ ${message}`;
        this.render();
        // Desactivar modo silencioso cuando la barra termina con error
        global.PROGRESS_ACTIVE = false;
        process.stdout.write('\n');
...
class ProgressFactory {
    static createServiceProgressBar(service, total, message = '') {
        // Verificar si hay múltiples servicios activos (TEST mode)
        const isMultiService = process.env.TEST_GPS === 'true' &&
                              (process.env.TEST_VOZ === 'true' || process.env.TEST_ELIOT === 'true');

        if (isMultiService) {
            // Usar ProgressManager para manejo centralizado
            const ProgressManager = require('./ProgressManager');
            const manager = ProgressManager.getInstance();

            // Registrar servicio en el manager
            manager.register(service, total, message);

            // Retornar proxy que delega al manager
            return {
                update: (current, newMessage) => manager.update(service, current, newMessage),
                complete: (finalMessage) => manager.complete(service, finalMessage),
                fail: (errorMessage) => manager.fail(service, errorMessage),
                startTime: Date.now()
            };
        }

        // Modo single service - usar ProgressBar tradicional
        const serviceConfig = {
            GPS: {
                service: 'GPS',
                prefix: '🟢 GPS: ',
